...
class NewsFetcher:
    ...
    def fetch(self, symbol: str) -> List[str]:
        """Return up to max_per_ticker headline strings for symbol. Empty list on failure."""
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                with urllib.request.urlopen(req, timeout=self.timeout) as resp:
                    raw = resp.read()
                headlines = _parse_rss(raw, self.max_per_ticker)
                logger.info(f"News: {symbol} → {len(headlines)} headlines")
                return headlines
    ...
    def fetch_many(self, symbols: List[str]) -> Dict[str, List[str]]:
        """Fetch headlines for multiple symbols. Returns {symbol: [headline, ...]}."""
        return {symbol: self.fetch(symbol) for symbol in symbols}


def _parse_rss(raw: bytes, max_items: int) -> List[str]:
    try:
