...
.hdr h1{font-size:1.6rem;font-weight:700;letter-spacing:-0.5px}
...
.card h3{font-size:.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.6px;color:#636e72;margin-bottom:16px}
...
<div class="wrap">
  ...
  <div class="g2">
    <div class="card">
      <h3>Portfolio Value by Symbol (SGD)</h3>
      <div class="chart-wrap"><canvas id="c-value"></canvas></div>
    </div>
    <div class="card">
      <h3>Unrealised Gain / Loss (SGD)</h3>
      <div class="chart-wrap"><canvas id="c-gain"></canvas></div>
    </div>
  </div>
  ...
  <div class="g3">
    <div class="card">
      ...
      <div class="chart-wrap" style="max-width:280px;margin:0 auto"><canvas id="c-alloc"></canvas></div>
    </div>
    <div class="card">
      <h3>By Account Type</h3>
      <div class="chart-wrap" style="max-width:280px;margin:0 auto"><canvas id="c-acct"></canvas></div>
    </div>
    <div class="card">
      <h3>By Currency</h3>
      <div class="chart-wrap" style="max-width:280px;margin:0 auto"><canvas id="c-ccy"></canvas></div>
    </div>
  </div>

  <div class="g2">
    <div class="card span2">
      <h3>Annualised Return by Lot (CAGR, contract dates known &amp; ≥ 1 yr held)</h3>
      <div class="chart-wrap"><canvas id="c-cagr"></canvas></div>
    </div>
  </div>

  <div class="card">
    <h3>All Positions — click any column header to sort</h3>
    <div class="tbl-wrap">
      <table id="tbl">
        <thead>
          <tr>
...
function sym(ccy){ return CCY_SYM[ccy] || (D.meta.base_currency+' '); }
function fmtV(v, ccy){ return v==null ? '—' : (sym(ccy||D.meta.base_currency))+Math.abs(v).toLocaleString('en-SG',{maximumFractionDigits:0}); }
...
// KPI Cards
const s = D.summary;
const gainClass = s.total_gain >= 0 ? 'pos' : 'neg';
document.getElementById('kpi-cards').innerHTML = [
  {lbl:'Total Value', val: fmtV(s.total_value), cls:''},
  {lbl:'Total Gain', val: (s.total_gain>=0?'+':'')+fmtV(s.total_gain)+' ('+fmtP(s.total_gain_pct)+')', cls:gainClass},
  {lbl:'Positions', val: D.meta.n_lots+' lots / '+D.meta.n_symbols+' symbols', cls:''},
  {lbl:'Currency', val: D.meta.base_currency, cls:''},
...
function hbar(id, labels, values, colors, xLabel){
  new Chart(document.getElementById(id),{
    type:'bar',
    data:{labels, datasets:[{data:values, backgroundColor:colors, borderRadius:4, borderSkipped:false}]},
    options:{
      indexAxis:'y', responsive:true,
      plugins:{legend:{display:false}, tooltip:{callbacks:{label:ctx=>fmtV(ctx.raw)}}},
      scales:{x:{grid:{color:'#f1f2f6'}, ticks:{callback:v=>fmtV(v)},
    ...
    options:{
      ...
      plugins:{legend:{position:'bottom', labels:{boxWidth:12, padding:10}},
               tooltip:{callbacks:{label:ctx=>{
                 const pct=(ctx.raw/ctx.dataset.data.reduce((a,b)=>a+b,0)*100).toFixed(1);
                 return ctx.label+': '+fmtV(ctx.raw)+' ('+pct+'%)';
               }}}}
    }
  });
}

// ── Chart 1: Value by symbol ──────────────────────────────────────────────
const bySymVal = D.by_symbol.slice().sort((a,b)=>b.total_value_sgd-a.total_value_sgd);
hbar('c-value',
  bySymVal.map(s=>s.symbol),
  bySymVal.map(s=>s.total_value_sgd),
  PALETTE.slice(0, bySymVal.length),
);

// ── Chart 2: Gain by symbol ───────────────────────────────────────────────
const bySymGain = D.by_symbol.filter(s=>s.total_gain_sgd!=null)
  .slice().sort((a,b)=>b.total_gain_sgd-a.total_gain_sgd);
hbar('c-gain',
  bySymGain.map(s=>s.symbol+' ('+fmtP(s.gain_pct)+')'),
  bySymGain.map(s=>s.total_gain_sgd),
  bySymGain.map(s=>s.total_gain_sgd>=0?'rgba(39,174,96,.75)':'rgba(231,76,60,.75)'),
);

// ── Chart 3: Allocation donut ─────────────────────────────────────────────
donut('c-alloc',
  D.by_symbol.map(s=>s.symbol),
  D.by_symbol.map(s=>s.total_value_sgd),
  PALETTE.slice(0, D.by_symbol.length));

// ── Chart 4: By account ───────────────────────────────────────────────────
const ACCT_COLORS = {cash:'#74b9ff', CPF:'#a29bfe', SRS:'#fdcb6e', taxable:'#55efc4'};
donut('c-acct',
  D.by_account.map(a=>a.name),
  D.by_account.map(a=>a.value_sgd),
  D.by_account.map(a=>ACCT_COLORS[a.name]||'#dfe6e9'));

// ── Chart 5: By currency ──────────────────────────────────────────────────
const CCY_COLORS = {SGD:'#00b894', USD:'#0984e3', TWD:'#e17055', GBp:'#6c5ce7', EUR:'#fdcb6e'};
donut('c-ccy',
  D.by_currency.map(c=>c.name),
  D.by_currency.map(c=>c.value_sgd),
  D.by_currency.map(c=>CCY_COLORS[c.name]||'#b2bec3'));

// ── Chart 6: CAGR ─────────────────────────────────────────────────────────
const cagrLots = D.lots
  .filter(l=>l.ann_return_pct!=null && l.years_held!=null && l.years_held>=1)
...
if(cagrLots.length){
  new Chart(document.getElementById('c-cagr'),{
    ...
    data:{
      ...
      datasets:[{
        data: cagrLots.map(l=>l.ann_return_pct),
    ...
    options:{
      ...
      plugins:{legend:{display:false},
               tooltip:{callbacks:{label:ctx=>{
                 ...
                 return [`CAGR: ${fmtP(ctx.raw)}`,`Held: ${fmtN(l.years_held,1)} yrs`,
                         `Gain: ${fmtP(l.gain_pct)}`];
               }}}},
      scales:{x:{grid:{color:'#f1f2f6'},ticks:{callback:v=>fmtP(v)}},
              y:{grid:{display:false}}}
    }
  });
} else {
